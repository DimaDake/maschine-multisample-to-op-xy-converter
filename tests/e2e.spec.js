...
test('should enable "Generate Single ZIP" button after selecting a library', async ({ page }) => {
  const mockLibraryStructure = {
    name: 'Native Instruments',
    kind: 'directory',
    entries: {
  ...
  await page.goto('/index.html');
  await page.locator('#select-library-button').click();
  await expect(page.locator('#file-count')).toHaveText('1 instrument(s) found.', { timeout: 20000 });
  await expect(page.locator('#convert-button')).toBeEnabled();
  await expect(page.locator('#convert-button')).not.toHaveClass(/bg-gray-400/);
  await expect(page.locator('#convert-button')).toHaveClass(/bg-green-600/);
});